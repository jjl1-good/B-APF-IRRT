...
%RRTSTAR2D Standard uniform-sampling RRT* baseline.
...
for it=1:opts.maxIter
    ...
    for z=1:numel(cand)
        j=cand(z);
        if C(j)+dist(j)<cc && collisionChecking(X(j,:),xn,env.squareAll,env.round)
            par=j; cc=C(j)+dist(j);
        end
    end
    ...
    for z=1:numel(cand)
        ...
        if j~=par && newCost<C(j) && collisionChecking(xn,X(j,:),env.squareAll,env.round)
            P(j)=n; C(j)=newCost;
        end
    end
    dg=norm(xn-env.goal);
    if dg<=opts.step*1.5 && collisionChecking(xn,env.goal,env.squareAll,env.round)
        gc=cc+dg;
        if gc<best, best=gc; goalIdx=n; end
    end
end
if goalIdx>0
    q=goalIdx; path=env.goal;
    while q>1, path(end+1,:)=X(q,:); q=P(q); end
    path(end+1,:)=env.start; path=flipud(path); path=shortcut(path,env);
else
    path=zeros(0,2);
end
