...
title('RRT*', 'FontSize', 12, 'FontWeight', 'bold');
...
% 初始化可视化对象
rrt_tree_plot = plot3(NaN, NaN, NaN, 'b-', 'LineWidth', 0.5, 'Color', [0.7 0.7 1]);
rrt_nodes_plot = scatter3(NaN, NaN, NaN, 20, 'c', 'filled', 'MarkerFaceAlpha', 0.5);
final_path_plot = plot3(NaN, NaN, NaN, 'r-', 'LineWidth', 4);
...
for iter = 1:Maxiter
    ...
    % 定期更新可视化
    if mod(iter, visual_update_interval) == 0
...
if Goal_index > 0 && Goal_index <= node_count
    ...
    % 在终点添加标记
    scatter3(goalPoint(1), goalPoint(2), goalPoint(3), 150, 'y', 'filled', 'MarkerEdgeColor', 'k', 'LineWidth', 2);
end

% 添加图例
legend('起点', '终点', 'RRT*树', 'RRT*节点', '最终路径', '优化目标', 'Location', 'northeast');
...
function update_tree_visualization(Tv_x, Tv_y, Tv_z, Tv_pre, node_count, tree_plot, nodes_plot)
    start_idx = max(1, node_count - 1000); % 只显示最近1000个节点以提升性能
    tree_x = [];
    tree_y = [];
    tree_z = [];
