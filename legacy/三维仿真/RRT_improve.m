...
% Path = RRTTD(startPoint,axisLWH,axisStart,cubeInfo,cylinderInfo,sphereInfo,goalPoint);
%% 三维RRT算法寻找路径点
%% 变量定义
Maxiter = 5000; %最大迭代次数
iter = 0; %当前迭代次数
...
for iter = 1 : Maxiter
   ...
   %% 寻找范围内的点 
    ...
    for k3 = 1:length(Tv) % should not use size() function
       ...
       Tv(count).cos=tempcos;
       line([Tv(temppre).x  newp.x],[Tv(temppre).y  newp.y],[Tv(temppre).z  newp.z],'LineWidth',1,'Color','b');
       pause(0.01);
       %% 重布线
        for k4=1:length(rewirep)
            if co(rewirep(k4))+tempcos<Tv(rewirep(k4)).cos
                Tv(rewirep(k4)).cos = co(rewirep(k4))+tempcos;
                Tv(reworep(k4)).pre = count4;
            end
        end 
