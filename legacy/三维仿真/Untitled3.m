...
% 检查是否到达目标
final_distance = norm([b_x(end)-goalPoint(1), b_y(end)-goalPoint(2), b_z(end)-goalPoint(3)]);
if final_distance > Thr
    fprintf('警告：BUG-APF未到达目标，最终距离: %.2f\n', final_distance);
    % 将终点直接添加到路径中
    b_x(end+1) = goalPoint(1);
    b_y(end+1) = goalPoint(2);
    b_z(end+1) = goalPoint(3);
    bug_apf_length = bug_apf_length + final_distance;
end
...
grid_count = zeros(maxgridx, maxgridy, maxgridz);

% 初始化所有网格单元
for ix = 1:maxgridx
    for iy = 1:maxgridy
        for iz = 1:maxgridz
            gridin{ix,iy,iz} = zeros(1, 100);
        end
    end
end

% 添加起点到网格
gx1 = min(max(ceil(Tv_x(1)/gw), 1), maxgridx);
gy1 = min(max(ceil(Tv_y(1)/gw), 1), maxgridy);
gz1 = min(max(ceil(Tv_z(1)/gw), 1), maxgridz);
grid_count(gx1, gy1, gz1) = 1;
gridin{gx1, gy1, gz1}(1) = 1;
...
for k = 2:length(b_x)
    ...
    Tv_z(o) = b_z(k);
    Tv_pre(o) = o - 1;
    step_distance = norm([b_x(k)-b_x(k-1), b_y(k)-b_y(k-1), b_z(k)-b_z(k-1)]);
    Tv_cos(o) = Tv_cos(o-1) + step_distance;
    
    % 更新网格
    gx = min(max(ceil(Tv_x(o)/gw), 1), maxgridx);
    gy = min(max(ceil(Tv_y(o)/gw), 1), maxgridy);
    gz = min(max(ceil(Tv_z(o)/gw), 1), maxgridz);
    
    grid_count(gx, gy, gz) = grid_count(gx, gy, gz) + 1;
    current_count = grid_count(gx, gy, gz);
    
    if current_count > length(gridin{gx, gy, gz})
        gridin{gx, gy, gz} = [gridin{gx, gy, gz}, zeros(1, 50)];
    end
    gridin{gx, gy, gz}(current_count) = o;
end

Goal_index = o;
fprintf('初始树节点数: %d，初始路径长度: %.2f\n', o, Tv_cos(Goal_index));
...
for iter = 1:Maxiter
    if mod(iter, progress_interval) == 0
        fprintf('进度: %d/%d (%.1f%%) 当前节点数: %d，最佳路径长度: %.2f\n', ...
            iter, Maxiter, iter/Maxiter*100, o, best_length);
        
        % 更新进度显示
        subplot(2,3,3);
        set(progress_text, 'String', sprintf('IRRT*优化中...\n迭代: %d/%d\n节点数: %d\n当前最优: %.2f', ...
            iter, Maxiter, o, best_length));
        
        % 更新统计信息
        subplot(2,3,6);
        stats_str = sprintf('迭代次数: %d/%d\n树节点数: %d\n当前最优: %.2f\n初始长度: %.2f', ...
            iter, Maxiter, o, best_length, bug_apf_length);
        set(stats_text, 'String', stats_str);
        
        % 更新树可视化
        if o > 1
            recent_indices = max(1, o-200):o;
            tree_x = []; tree_y = []; tree_z = [];
            for idx = recent_indices
                if Tv_pre(idx) > 0
                    tree_x = [tree_x, Tv_x(Tv_pre(idx)), Tv_x(idx), NaN];
                    tree_y = [tree_y, Tv_y(Tv_pre(idx)), Tv_y(idx), NaN];
                    tree_z = [tree_z, Tv_z(Tv_pre(idx)), Tv_z(idx), NaN];
                end
            end
            set(rrt_tree_plot, 'XData', tree_x, 'YData', tree_y, 'ZData', tree_z);
            set(rrt_nodes_plot, 'XData', Tv_x(1:o), 'YData', Tv_y(1:o), 'ZData', Tv_z(1:o));
        end
    ...
    % 改进的采样策略
        ...
        % 椭球采样
        cb = max(Tv_cos(best_goal_index)/2, 1);
        ...
        if randp_x>= cubeInfo.axisX(k)&& randp_x<cubeInfo.axisX(k)+cubeInfo.lengthx(k)
            if randp_y>= cubeInfo.axisY(k)&& randp_y<cubeInfo.axisY(k)+cubeInfo.lengthy(k)
                if randp_z>= cubeInfo.axisZ(k)&& randp_z<cubeInfo.axisZ(k)+cubeInfo.lengthz(k)
                    valid_point = false; % invalid node
                    break
                end
            end
        end
    ...
    % 寻找最近节点
    tgrid_x = ceil(randp_x/gw);
    tgrid_y = ceil(randp_y/gw);
    tgrid_z = ceil(randp_z/gw);
    x_near = [];
    c = 1;
    while isempty(x_near)
        for i_grid = max(tgrid_x-c,1):min(tgrid_x+c,maxgridx)
            for j_grid = max(tgrid_y-c,1):min(tgrid_y+c,maxgridy)
                for k_grid = max(tgrid_z-c,1):min(tgrid_z+c,maxgridz)
                    if grid_count(i_grid, j_grid, k_grid) > 0
                        for idx = 1:grid_count(i_grid, j_grid, k_grid)
                            node_idx = gridin{i_grid, j_grid, k_grid}(idx);
                            if node_idx > 0
                                x_near(end+1) = node_idx;
                            end
                        end
                    end
                end
  ...
  %% 完整的IRRT*节点扩展过程 - 包含重选父节点

% 在生成新节点后，添加到树之前，先进行重选父节点优化
new_x_candidate = new_x;
new_y_candidate = new_y; 
new_z_candidate = new_z;
newgridx = ceil(new_x/gw);
newgridy = ceil(new_y/gw);
newgridz = ceil(new_z/gw);
% 步骤1: 在附近节点中寻找更优的父节点
best_parent = nearest_idx;
best_cost = Tv_cos(nearest_idx) + step; % 当前最小代价
min_distance = step;

% 搜索新节点附近的潜在父节点
candidate_parents = [];
search_radius_parent = ceil(rewire_Thr/gw);

for i_grid = max(1, newgridx-search_radius_parent):min(maxgridx, newgridx+search_radius_parent)
    for j_grid = max(1, newgridy-search_radius_parent):min(maxgridy, newgridy+search_radius_parent)
        for k_grid = max(1, newgridz-search_radius_parent):min(maxgridz, newgridz+search_radius_parent)
            if grid_count(i_grid, j_grid, k_grid) > 0
                for idx = 1:grid_count(i_grid, j_grid, k_grid)
                    candidate_idx = gridin{i_grid, j_grid, k_grid}(idx);
                    if candidate_idx > 0 && candidate_idx ~= nearest_idx
                        % 计算距离
                        ...
                        if dist_candidate <= rewire_Thr
                            candidate_parents(end+1) = candidate_idx;
                        end
                    end
                end
            end
...
Tv_x(o) = new_x_candidate;
Tv_y(o) = new_y_candidate;
Tv_z(o) = new_z_candidate;
Tv_pre(o) = best_parent;
Tv_cos(o) = best_cost; % 使用优化后的代价

% 更新网格
gx = min(max(ceil(new_x_candidate/gw), 1), maxgridx);
gy = min(max(ceil(new_y_candidate/gw), 1), maxgridy);
gz = min(max(ceil(new_z_candidate/gw), 1), maxgridz);
...
newgridx = ceil(new_x_candidate/gw);
newgridy = ceil(new_y_candidate/gw);
newgridz = ceil(new_z_candidate/gw);

x_new_near = [];
search_radius_new = 1;
max_search_radius_new = ceil(rewire_Thr/gw) + 1;

while search_radius_new <= max_search_radius_new
    for i_grid = max(1, newgridx-search_radius_new):min(maxgridx, newgridx+search_radius_new)
        for j_grid = max(1, newgridy-search_radius_new):min(maxgridy, newgridy+search_radius_new)
            for k_grid = max(1, newgridz-search_radius_new):min(maxgridz, newgridz+search_radius_new)
                if grid_count(i_grid, j_grid, k_grid) > 0
                    for idx = 1:grid_count(i_grid, j_grid, k_grid)
                        node_idx = gridin{i_grid, j_grid, k_grid}(idx);
                        if node_idx > 0 && node_idx ~= o && node_idx ~= best_parent
                            % 计算距离
                            dx = Tv_x(node_idx) - new_x_candidate;
                            dy = Tv_y(node_idx) - new_y_candidate;
                            dz = Tv_z(node_idx) - new_z_candidate;
                            dist = sqrt(dx*dx + dy*dy + dz*dz);
                            if dist <= rewire_Thr
                                x_new_near(end+1) = node_idx;
                            end
                        end
                    end
                end
    ...
    search_radius_new = search_radius_new + 1;
end
...
for k = 1:length(x_new_near)
    ...
    if new_cost < Tv_cos(candidate_idx)
        ...
        if ~(cubeFlag_check || cylinderFlag_check || sphereFlag_check)
            ...
            Tv_pre(candidate_idx) = o;
            old_cost = Tv_cos(candidate_idx);
            Tv_cos(candidate_idx) = new_cost;
            
%             % 更新候选节点后代的代价
%             updateNodeCost(candidate_idx, old_cost, new_cost,Tv_cos,Tv_pre);
%             
%             % 从原父节点的网格中移除候选节点
%             if old_parent > 0
%                 old_gx = min(max(ceil(Tv_x(old_parent)/gw), 1), maxgridx);
%                 old_gy = min(max(ceil(Tv_y(old_parent)/gw), 1), maxgridy);
%                 old_gz = min(max(ceil(Tv_z(old_parent)/gw), 1), maxgridz);
%                 
%                 % 找到并移除候选节点
%                 idx_to_remove = find(gridin{old_gx, old_gy, old_gz} == candidate_idx, 1);
%                 if ~isempty(idx_to_remove)
%                     gridin{old_gx, old_gy, old_gz}(idx_to_remove) = 0;
%                     % 可以在这里压缩网格数组（可选）
%                 end
%             end
        end
    ...
    % 检查是否接近目标并更新最佳路径
    dist_to_goal = norm([new_x, new_y, new_z] - goalPoint);
    if dist_to_goal < Thr
        current_cost = Tv_cos(o) + dist_to_goal;
        if current_cost < best_length
            best_length = current_cost;
            best_goal_index = o;
            Tv_cos(Goal_index)=current_cost;
            Tv_pre(Goal_index) = o;
            fprintf('找到更好路径! 长度: %.2f\n', best_length);
        end
    end
end
...
if best_goal_index > 1
    ...
    % 在终点添加特殊标记
    scatter3(Tv_x(best_goal_index), Tv_y(best_goal_index), Tv_z(best_goal_index), 150, 'y', 'filled', 'MarkerEdgeColor', 'k', 'LineWidth', 2);
    
    % 更新图例
    legend('起点', '终点', '立方体障碍物', '圆柱障碍物', '球体障碍物', ...
           'BUG-APF路径', '当前位置', '合力方向', 'RRT*树', 'RRT*节点', '最终路径', '找到的目标', ...
...
set(progress_text, 'String', sprintf('算法完成!\n总时间: %.2f秒\n最终路径长度: %.2f\n优化效果: %.2f%%', ...
    t2, final_path_length, improvement));

subplot(2,3,6);
final_stats = sprintf('总运行时间: %.2f秒\n最终路径长度: %.2f\n初始路径长度: %.2f\n优化效果: %.2f%%\n总节点数: %d\nBUG-APF步数: %d\nRRT*迭代: %d', ...
    t2, final_path_length, bug_apf_length, improvement, o, length(b_x), Maxiter);
set(stats_text, 'String', final_stats);

% 添加完成提示
subplot(2,3,[1,2,4,5]);
text(axisLWH(1)/2, axisLWH(2)/2, axisLWH(3)+10, ...
    sprintf('算法完成! 时间: %.2f秒\n路径优化: %.2f%%', t2, improvement), ...
