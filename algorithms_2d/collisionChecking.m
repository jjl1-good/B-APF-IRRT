function feasible = collisionChecking(point1, point2, squareObstacles, roundObstacles, margin)
%COLLISIONCHECKING 检查二维线段是否与障碍物相交（解析精确判定）。
%
%   feasible = collisionChecking(point1, point2, squareObstacles, roundObstacles)
%   feasible = collisionChecking(point1, point2, squareObstacles, roundObstacles, margin)
%
% 返回值遵循当前二维算法中的调用约定：
...
% margin            可选安全裕度，将障碍物向外膨胀 margin（默认 0），
%                   用于最小安全间隙检验。
%
% 判定方法：矩形使用 Liang-Barsky 解析裁剪，圆使用点到线段距离的解析
% 表达式，均为精确判定（无采样分辨率问题），边界接触计为碰撞。

% 默认认为路径不可行；参数异常时直接返回不可行，避免误放行。
feasible = false;

if nargin < 5 || isempty(margin), margin = 0; end
...
for k = 1:size(squareObstacles, 1)
    ...
    xmin = rect(1) - margin;
    xmax = rect(1) + rect(3) + margin;
    ymin = rect(2) - margin;
    ymax = rect(2) + rect(4) + margin;
    if segmentIntersectsRectangle(point1, point2, xmin, xmax, ymin, ymax)
        return;
    end
end

% 线段到圆心的最短距离不大于半径（膨胀 margin）即视为碰撞。
for k = 1:size(roundObstacles, 1)
    circle = roundObstacles(k, 1:3);
    if any(~isfinite(circle)) || circle(3) < 0
        return;
    end
    if pointSegmentDistanceSquared(point1, point2, circle(1:2)) <= (circle(3) + margin)^2
        return;
    end
end
