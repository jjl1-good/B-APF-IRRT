...
if ~isfield(opts, 'gridStep')
    % 小地图用 1 单位栅格以解析极窄通道；大地图用 2 单位栅格控制 A* 计算规模。
    if max(env.bounds(2) - env.bounds(1), env.bounds(4) - env.bounds(3)) <= 200
        opts.gridStep = 1;
    else
        opts.gridStep = 2;
    end
end
if ~isfield(opts,'maxNodes'), opts.maxNodes = 400000; end
step = opts.gridStep; b = env.bounds;
% 半整数栅格（0.5+step 的相位）：对整数坐标对齐的障碍物边界能更好地
% 表示 1 单位宽的缝隙，避免因采样恰好落在边界（含边界视为碰撞）而失真。
xs = (b(1) + step / 2):step:(b(2) - step / 2);
ys = (b(3) + step / 2):step:(b(4) - step / 2);
